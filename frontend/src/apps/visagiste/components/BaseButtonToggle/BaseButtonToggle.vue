...
// Components
import BaseButtonGroup, { useBaseButtonGroupProps } from '../BaseButtonGroup/BaseButtonGroup.vue'
...
export const BaseButtonToggleSymbol: InjectionKey<GroupProvide> = Symbol.for('visagiste:base-button-toggle')
...
export default defineComponent({
  ...
  setup (props, { slots }) {
    const { isSelected, next, prev, select, selected } = useGroup(props, BaseButtonToggleSymbol)
    return () => {
      ...
      return h(
        ...
        {
          default: () => slots.default?.(isSelected, next, prev, select, selected)
        }
      )
    }
  }
