...
// Utilities
import {consoleError, consoleWarn, getObjectValueByPath} from "@/apps/visagiste/utils";

// Locales

// Types
import type {Ref} from "vue";
import type {LocaleInstance, LocaleMessages, LocaleOptions} from "@/apps/visagiste/composables/locale";
import {values} from "lodash";
import {useProxiedModel} from "@/apps/visagiste/composables/proxiedModel";
import {ref, shallowRef, watch} from "vue";
import en from "@/apps/visagiste/locale/en";



const LANG_PREFIX = '$visagiste.'
...
const createTranslateFunction = (
  ...
  return (key: string, ...params: unknown[]) => {
    ...
    if (!str) {
      consoleError(`Translation key "${key}" not found in "${current.value}", trying fallback locale`)
      str = key
    }
...
export function createVisagisteAdapter (options?: LocaleOptions): LocaleInstance {
