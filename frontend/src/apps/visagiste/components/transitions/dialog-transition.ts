...
export const useBaseDialogTransitionProps = propsFactory({
  target: [Object, Array] as PropType<HTMLElement | [x: number, y: number]>,
}, 'base-dialog-transition')

export default defineComponent({
  name: 'BaseDialogTransition',
  props: useBaseDialogTransitionProps(),
  setup (props, { slots }) {
    const functions = {
...
function getDimensions (target: HTMLElement | [x: number, y: number], el: HTMLElement) {
  ...
  const [anchorSide, anchorOffset] = getComputedStyle(el).getPropertyValue('--base-overlay-anchor-origin').split(' ')
