...
import { useBorder } from "@/apps/visagiste/composables/border";
import { useElevation } from "@/apps/visagiste/composables/elevation";
...
const hasTitle = computed(() => !!(props.title || slots.image));
const hasImage = computed(() => !!(slots.image || props.image));
const extension = computed(() => slots.extension?.());
isExtended.value = computed(() => !!(props.extended || extension.value)).value;
