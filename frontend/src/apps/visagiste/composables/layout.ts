...
export const VisagisteLayoutKey: InjectionKey<LayoutProvide> =
  Symbol.for("visagiste:layout");
export const VisagisteLayoutItemKey: InjectionKey<{ id: string }> = Symbol.for(
  "visagiste:layout-item",
);

const ROOT_ZINDEX = 1000;
export const useLayoutProps = propsFactory(
  {
    overlaps: {
      type: Array,
      default: () => [],
...
// Composables
export const useLayoutItemProps = propsFactory(
  {
    name: {
      type: String,
    },
...
export function useLayout() {
  const layout = inject(VisagisteLayoutKey);

  if (!layout) throw new Error("[Visagiste] Could not find injected layout");
...
export function useLayoutItem(options: {
  ...
  provide(VisagisteLayoutItemKey, { id });

  const isKeepAlive = shallowRef(false);
  onDeactivated(() => (isKeepAlive.value = true));
  onActivated(() => (isKeepAlive.value = false));

  const { layoutItemStyles, layoutItemScrimStyles } = layout.register(vm, {
    ...options,
    active: computed(() => (isKeepAlive.value ? false : options.active.value)),
    id,
  });
...
export function createLayout(props: {
  ...
  const parentLayout = inject(VisagisteLayoutKey, null);
  ...
  const mainStyles = computed<CSSProperties>(() => {
    return {
      "--base-layout-left": convertToUnit(mainRect.value.left),
      "--base-layout-right": convertToUnit(mainRect.value.right),
      "--base-layout-top": convertToUnit(mainRect.value.top),
      "--base-layout-bottom": convertToUnit(mainRect.value.bottom),
  ...
  provide(VisagisteLayoutKey, {
    register: (
      ...
      const layoutItemStyles = computed<CSSProperties>(() => {
        ...
        if (!item)
          throw new Error(`[Visagiste] Could not find layout item "${id}"`);
  ...
  const layoutClasses = computed(() => [
    "base-layout",
    { "base-layout--full-height": props.fullHeight },
  ]);
